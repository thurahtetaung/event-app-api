...
redisClient.connect().then(async () => {
  ...
  try {
    ...
    logger.info(`Redis test key set and retrieved successfully: ${testResult}`);
  } catch (error) {
...
export async function reserveTicket(ticketId: string, userId: string): Promise<boolean> {
  try {
    ...
    logger.error(`Redis error details: ${JSON.stringify({
      ticketId,
      userId,
...
export async function isTicketReserved(ticketId: string, userId?: string): Promise<boolean> {
  try {
    const key = TICKET_PREFIX + ticketId;
...
export async function getReservedTicketCount(ticketTypeId: string): Promise<number> {
  try {
    ...
    logger.error(`Error counting reserved tickets for type ${ticketTypeId}: ${error}`);
    return 0;
  }
}
